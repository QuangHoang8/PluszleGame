...
import { getTimeMinuteSencond } from "../01.common/common.js";
import { RenderInGameDisplay } from "../03.in-game/renderInGameDisplay.js";
import { renderStartGameDisplay } from "../02.start-game/renderStartGameDisplay.js";
...
export class RenderEndGameDisplay {
  ...
  getTimeMinuteSencond() {
    const { hour, second, minute } = getTimeMinuteSencond(this.numberOfSecond);
    this.hour = hour;
    this.minute = second;
    this.second = minute;
  }
  ...
  renderSupportResult() {
    ...
    parentSupportDiv.classList.add(
      "flex",
      "flex-around",
      "button-support-result"
    );
  ...
  renderButton() {
    ...
    parentRestartDiv.onclick = () => this.restartGame();
    ...
    parentExitDiv.onclick = () => this.endGame();
  ...
  restartGame() {
    ...
    const renderInGameDisplay = new RenderInGameDisplay({ rows: this.rows });
